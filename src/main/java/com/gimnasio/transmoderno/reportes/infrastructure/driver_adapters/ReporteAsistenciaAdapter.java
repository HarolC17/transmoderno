...
public class ReporteAsistenciaAdapter implements ReporteAsistenciaPort {
    ...
    @Override
    public List<ReporteAsistenciaDetalle> obtenerDetalleAsistencia(Long rutaId,
                                                                   String programaAcademico,
                                                                   Integer semestre,
                                                                   String estamento,
                                                                   LocalDate fechaInicio,
                                                                   LocalDate fechaFin) {
        StringBuilder jpql = new StringBuilder("""
            SELECT p.nombreCompleto, p.numeroIdentificacion, p.correoInstitucional,
                   p.programaAcademico, p.semestre, p.estamento, r.nombre, s.nombre, s.fecha
            FROM RegistroAsistenciaData a
            JOIN SesionData s ON s.id = a.sesionId
            JOIN RutaData r ON r.id = s.rutaId
            JOIN ParticipanteData p ON p.id = a.participanteId
